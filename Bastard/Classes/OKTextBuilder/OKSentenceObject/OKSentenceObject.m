...
- (id) initWithSentence:(NSString *)aSentence withFont:(OKTessFont*)aFont
{
    self = [self init];
	if (self != nil)
	{
        ...
        tessFont = aFont;
        width = [aFont getWidthForString:sentence];
        height = [aFont getHeightForString:sentence];
        ...
        for(OKWordObject *wordObj in wordObjects)
        {
            ...
            for(OKCharObject *charObj in wordObj.charObjects)
            {
                ...
                charPoint.x += [aFont getXAdvanceForString:charObj.charObj] - (prevCharObj == nil ? 0 : [aFont getKerningForLetter:charObj.charObj withPreviousLetter:prevCharObj.charObj]);
                prevCharObj = charObj;
            }
            prevCharObj = nil;
            //space
            wordPoint.x += wordWidth + [aFont getWidthForString:@" "];
        }
    }
    return self;
}
...
- (void) setPosition:(OKPoint)aPos
{
    ...
    NSLog(@"Sentence %@ x: %f  y: %f", sentence, absPos.x, absPos.y);
    
    for(OKWordObject *word in wordObjects)
    {
        [word setPosition:absPos];
    }
...
- (void) drawSentence
{
    [tessFont drawStringAt:OKPointMake(x, y, 800.0) withString:sentence andDetail:3];
}
