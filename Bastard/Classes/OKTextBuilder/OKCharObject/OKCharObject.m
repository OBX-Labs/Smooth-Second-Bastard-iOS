...
- (id) initWithChar:(NSString*)aChar withFont:(OKTessFont*)aFont
{
    ...
    if (self != nil)
    {
        ...
        tessFont = aFont;
        width = [aFont getWidthForString:charObj];
        height = [aFont getHeightForString:charObj];
        
        OKCharDef* charDef = [aFont getCharDefForChar:charObj];
        xOffset = [charDef xOffset];
        yOffset = [charDef yOffset];
        
        origData = [tessFont getCharDefForChar:charObj];
...
- (void) drawChar
{
    if(visible)
    {
        [tessFont drawStringAt:OKPointMake(x, y, 800.0) withString:charObj andDetail:3];
